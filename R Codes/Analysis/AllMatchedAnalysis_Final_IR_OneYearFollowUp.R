...
# Group by outcome and subgroup, and then summarize both adjusted and unadjusted HRs in a single row
transformed_df_one_year <- results_df %>%
  select(outcome, subgroup, event_count_control, event_count_risperidone, person_time_control, person_time_risperidone, 
         incidence_rate_control_per_1000_PY, incidence_rate_risperidone_per_1000_PY, ARR, NNH, ci.l_NNH, ci.u_NNH, hr, ci.l, ci.u, model) %>%
  mutate(
    Adjusted_HR = ifelse(model == "adjusted", paste0(hr, " (", ci.l, ", ", ci.u, ")"), NA),
    Unadjusted_HR = ifelse(model == "unadjusted", paste0(hr, " (", ci.l, ", ", ci.u, ")"), NA)
  ) %>%
  # Pivot wider to have both Adjusted and Unadjusted HRs in the same row
  pivot_wider(names_from = model, values_from = c(hr, ci.l, ci.u)) %>%
  group_by(outcome, subgroup) %>%
  summarise(
    event_count_control = first(event_count_control),
    event_count_risperidone = first(event_count_risperidone),
    person_time_control = first(person_time_control),
    person_time_risperidone = first(person_time_risperidone),
    incidence_rate_control = first(incidence_rate_control_per_1000_PY),
    incidence_rate_risperidone = first(incidence_rate_risperidone_per_1000_PY),
    ARR = first(ARR),  # Take the first value of ARR (since it's constant for each outcome-subgroup pair)
    NNH = first(NNH),
...
write.csv(transformed_df_one_year, "FinalResults_one_year_new.csv", row.names = FALSE)
...
for (i in outcomes) {
  ...
  dat <- dat %>%
    mutate({{censdate_var}}:=pmin(as.Date(!!sym(outcome_var)), as.Date(cens_at), na.rm=TRUE),
           {{censvar_var}}:=ifelse(!is.na(as.Date(!!sym(outcome_var))) & !!sym(censdate_var)==!!sym(outcome_var), 1, 0),
           {{censtime_var}}:=as.numeric(difftime(!!sym(censdate_var), issuedate, unit="days"))/7,
    )
}
...
fit_cox_model <- function(data, outcome, subgroup, model_type) {
  ...
  event_count_control <- sum(data[[paste0(outcome, "_censvar")]] == 1 & data$risperidone == 0)
  person_time_control <- sum(pmin(data[[paste0(outcome, "_censtime_yrs")]], 12 / 52)[data$risperidone == 0])
  incidence_rate_control_per_1000_PY <- (event_count_control / person_time_control) * 1000
  
  event_count_risperidone <- sum(data[[paste0(outcome, "_censvar")]] == 1 & data$risperidone == 1)
  person_time_risperidone <- sum(pmin(data[[paste0(outcome, "_censtime_yrs")]], 12 / 52)[data$risperidone == 1])
  incidence_rate_risperidone_per_1000_PY <- (event_count_risperidone / person_time_risperidone) * 1000
...
write.csv(transformed_df_12_weeks, "FinalResults_12_weeks_new.csv", row.names = FALSE)
View(transformed_df_12_weeks)
options(max.print = nrow(transformed_df_12_weeks) * ncol(transformed_df_12_weeks))

print(transformed_df_12_weeks, n = Inf)
