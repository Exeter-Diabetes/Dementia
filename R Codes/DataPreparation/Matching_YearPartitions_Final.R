...
variables = c("patid","consid", "obsdate", "sex","regstartdate","earliest_obsdate", "regenddate", "death_composite_date","cprd_ddate", "lcd","region", "death_composite","gender_decode", "date_of_birth", "age_diagnosis","Diff_start_endOfFollowup","endoffollowup", "age_category", "year_of_diagnosis","risperidone", "other_drug_issuedate", "year_of_diagnosis", "issuedate", "three_months_after_other_psychotic", "Prescribed_other_antipsychotic_Prior", "age_risperidone", "ethnicity")


# Create an empty list to store matched data frames
matched_data_list <- list()
cumulative_matched_data <- data.frame()
sink("C:/Users/njc232/OneDrive - University of Exeter/Documents/MatchedResults_2004_2010_V2/summary_output.txt", append = TRUE)
...
for (year in 2004:2010) {
  ...
  ControlPartition_other_antiP <- ControlPartition_risp %>%
    ...
    filter(pmin(regenddate, death_composite_date, lcd, na.rm = TRUE) > paste0(year, "-01-01"))%>%
    collect() %>%
        mutate(
    ...
    rowwise() %>%
    mutate(
      pminDateTemp = pmin(regenddate, death_composite_date, lcd, na.rm = TRUE),
  ...
  FullData <- FullData %>%
    mutate(
      ...
      Survival_time = ifelse(is.na(stroke_compositeDate_post_first_stroke),
                             (as.numeric(pmin(death_composite_date, lcd, regenddate,na.rm = TRUE) - issuedate) / 365.25),
  ...
  pdf(paste0("C:/Users/njc232/OneDrive - University of Exeter/Documents/MatchedResults_2004_2010_V2/plot_", year, ".pdf"), width = 10, height = 8) 
  ...
  write.table(match.t, file = paste0("C:/Users/njc232/OneDrive - University of Exeter/Documents/MatchedResults_2004_2010_V2/match_t_", year, ".txt"), sep = "\t", row.names = FALSE)
  # Save matched data to the list
  file_name <- paste0("C:/Users/njc232/OneDrive - University of Exeter/Documents/MatchedResults_2004_2010_V2/matched_data_iteration_", year, ".txt")
...
write.table(all_matched_data, "C:/Users/njc232/OneDrive - University of Exeter/Documents/MatchedResults_2004_2010_V2/Joshua_all_matched_data.txt", sep = "\t", row.names = FALSE)

write.table(cumulative_matched_data, file = "C:/Users/njc232/OneDrive - University of Exeter/Documents/MatchedResults_2004_2010_V2/Joshua_cumulative_matched_data.txt",sep = "\t", row.names = FALSE)
