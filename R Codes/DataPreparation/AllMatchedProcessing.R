...
table <- read.table("C:/Users/njc232/OneDrive - University of Exeter/Documents/MatchedResults_2/Joshua_cumulative_matched_data.txt", header = TRUE, sep = "\t") 

table <- as.data.frame(table) %>%
  mutate(
    Stroke__within_year_after_1st_risperidone_presc	 = if_else(!is.na(obsdate_stroke) & !is.na(issuedate) & (as.numeric(as.Date(obsdate_stroke) - as.Date(issuedate))) >= 0 & (as.numeric(as.Date(obsdate_stroke) - as.Date(issuedate))) <= 365 & (comorbidity_stroke ==1), 1, 0)
...
table(table$post_index_date_stroke)

table <- table %>% mutate(
  stime=ifelse(is.na(post_index_date_first_stroke),
               (as.numeric(as.Date(pmin(cprd_ddate, lcd, regenddate,na.rm = TRUE)) - as.Date(issuedate)) / 365.25),
               (as.numeric(as.Date(post_index_date_first_stroke) - as.Date(issuedate)) / 365.25)))
...
table %>% group_by(risperidone) %>% dplyr::summarise(
  ...
  n_stroke=sum(post_index_date_stroke),
  pc_stroke=sum(post_index_date_stroke)/n()
)



##############################################################
fit <- survfit(Surv(stime, post_index_date_stroke) ~ risperidone, data = table)# , subset=Prescribed_other_antipsychotic_Prior==1)
# summary(fit)
ggsurv <- ggsurvplot(
  fit,
  data = table,
...
fit <- survfit(Surv(Survival_time, post_index_date_stroke) ~ risperidone, data = Under75)
# summary(fit)
ggsurv <- ggsurvplot(
  fit,
  data = Under75,
  ...
  tables.theme = theme_cleantable(),
  pval.size = 5,  # Set the size of the p-value text
  pval.coord = c(0.7, 13)  # Adjust the position of the p-value text
)
ggsurv$plot <- ggsurv$plot + ggtitle("Under 75 Age Group")
print(ggsurv)




Vars <- c("sex","risperidone", "age_diagnosis", "Stroke__within_year_after_1st_risperidone_presc", "pre_index_date_angina" , "pre_index_date_heartfailure", "BMI", "Prescribed_other_antipsychotic_Prior", "period_before_prescription",
...
binary_vars <- c("sex","risperidone", "Stroke__within_year_after_1st_risperidone_presc", "pre_index_date_angina" , "pre_index_date_heartfailure", "Prescribed_other_antipsychotic_Prior",
                 "comorbidity_myocardialinfarction" , "pre_index_date_stroke" ,"pre_index_date_tia" , "pre_index_date_falls" , "pre_index_date_lowerlimbfracture" , "pre_index_date_ihd" ,
                 "pre_index_date_pad" , "pre_index_date_af" , "pre_index_date_revasc" , "pre_index_date_qof_diabetes" , "pre_index_date_anxiety_disorders", "pre_index_date_fh_diabetes" ,
                 "pre_index_date_fh_premature_cvd" , "pre_index_date_pulmonary_embolism" , "pre_index_date_deep_vein_thrombosis" ,
...
FinalData$died <- as.numeric(as.character(FinalData$died))
...
FinalData %>% group_by(risperidone) %>% dplyr::summarise(
  ...
  max_s = max(FollowUp),
  n_stroke=sum(died),
  pc_stroke=sum(died)/n()
)

library(survminer)
fit <- survfit(Surv(FollowUp,died) ~ risperidone,data=FinalData)
ggsurv <- ggsurvplot(
...
fit <- survfit(Surv(FollowUp,died) ~ risperidone,data=AllDeathUnder75)
ggsurv <- ggsurvplot(
  fit,                     # survfit object with calculated statistics.
...
FinalData_year1 <- FinalData %>%
  mutate(
    died1 = ifelse(died==1 & FollowUp <=1,died,0),
...
library(survminer)
# fit <- survfit(Surv(time, status) ~ sex, data = lung)
fit <- survfit(Surv(fu1,died1) ~ risperidone,data=FinalData_year1)
ggsurv <- ggsurvplot(
  fit,                     # survfit object with calculated statistics.
...
ggsurv$plot <- ggsurv$plot + ggtitle("Death in first year")
...
FinalData <- FinalData %>%
  mutate(competing_death = ifelse(died == 1, 2, 0))
