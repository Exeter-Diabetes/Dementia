...
variables_of_interest <- c(
  ...
  "stroke_composite",            
  "death_composite",
  "Diff_start_endOfFollowup",                 
  "primary_death_stroke",
  "cause_of_death_stroke_type",  
  "ethnicity",
  "DiagnosedAfterCompositeDeath",
  "deprivation"
...
FullData$death_composite <- as.numeric(as.character(FullData$death_composite))

library(survminer)
# fit <- survfit(Surv(time, status) ~ sex, data = lung)
fit <- survfit(Surv(FollowUp,death_composite) ~ gender_decode,data=FullData)
ggsurv <- ggsurvplot(
  fit,                     # survfit object with calculated statistics.
  data = FullData,             # data used to fit survival curves.
...
unique_age_categories <- levels(FullData$age_category)

fit <- survfit(Surv(FollowUp, death_composite) ~ age_category, data = FullData)
...
# Fit survival model
fit <- survfit(Surv(FollowUp, death_composite) ~ 1, data = FullData)

# Extract survival times
surv_times <- FullData$FollowUp

# Calculate median, lower IQR (25th percentile), and upper IQR (75th percentile)
median_followup <- median(surv_times, na.rm = TRUE)
iqr_bounds <- quantile(surv_times, probs = c(0.25, 0.75), na.rm = TRUE)
lower_iqr <- iqr_bounds[1]
upper_iqr <- iqr_bounds[2]

cat("Median follow-up time (years):", median_followup, "\n")
cat("Lower IQR (25th percentile):", lower_iqr, "\n")
cat("Upper IQR (75th percentile):", upper_iqr, "\n")
