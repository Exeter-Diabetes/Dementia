...
# Data transformations for CVD and other features remain the same
dat <- FinalMatched %>%
  mutate(
...
# Additional transformations (handling cholesterol, hypertension) remain the same
dat <- dat %>%
  mutate(
...
dat <- dat %>%
  mutate(
    cens_at = pmin(as.Date(issuedate) + 84, as.Date(death_composite_date), lcd, regenddate, na.rm = TRUE),
    ...
    stroke_composite_post_index_date = ifelse(coalesce(Stroke, as.Date('9999-12-31')) > coalesce(Stroke_censor, as.Date('9999-12-31')), 0, stroke_composite_post_index_date)
  )
